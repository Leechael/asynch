...
| 54470 | `DBMS_MIN_PROTOCOL_VERSION_WITH_CHUNKED_PACKETS` | Addendum writes client send/receive chunked capability strings. Server hello includes server send/receive chunked capability strings. Default negotiation can remain `notchunked`. | both | Missing | Boundary tests for addendum write and hello read alignment |
| 54471 | `DBMS_MIN_REVISION_WITH_VERSIONED_PARALLEL_REPLICAS_PROTOCOL` | Server hello includes parallel replicas protocol version. Client addendum includes supported parallel replicas protocol version. | both | Missing | Boundary tests for hello/addendum below and at the gate |
